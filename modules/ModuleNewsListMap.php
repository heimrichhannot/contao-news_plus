...
class ModuleNewsListMap extends ModuleNewsPlus
{
    ...
    public function generateVenueMap($arrArticles)
    {
        ...
        $arrVenues = $this->getVenues($arrArticles);
    ...
    public function getVenues($arrArticles)
    {
        foreach ($arrArticles as $objArticle)
        {
            if (!$objArticle->addVenues)
            {
                continue;
            }

            $arrVenueIds = deserialize($objArticle->venues);
            if (empty($arrVenueIds))
            {
                continue;
            }


            foreach ($arrVenueIds as $intId)
            {
                if (($objField = FieldPaletteModel::findByPk($intId)) !== null)
                {
                    if (!$objField->venueSingleCoords)
                    {
                        continue;
                    }
